...
{
    return tr("4 - Interpolation par moyenne de Z");
}

// Step detailed description (tooltip of contextual menu)
QString LVOX_StepInterpolateDensityGrids::getStepDetailledDescription() const
{
    return tr("Cette étape fait une interpolation des valeurs de la grille de densité.");
}
...
void LVOX_StepInterpolateDensityGrids::createPostConfigurationDialog()
{
    ...
    configDialog->addInt(tr("effective rays (Nt - Nb) Threshold"),"",0,1000, _effectiveRayThresh,tr("Voxel with lower effective rays are considered as having no density."));
