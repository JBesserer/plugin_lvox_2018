...
void LVOX3_GenericCompute::initParser(mu::Parser &parser, const std::string& formula)
{
    ...
    for(char c = 'a'; c < 'z'; ++c) {
#if defined(_UNICODE)
        parser.DefineVar(QString(c).toStdWString(), m_variables[i++]);
#else
...
#if defined(_UNICODE)
    parser.DefineVar(QString("this").toStdWString(), &m_outputVariable);
    parser.SetExpr(QString::fromStdString(formula).toStdWString());
#else
    parser.DefineVar(QString("this").toStdString(), &m_outputVariable);
