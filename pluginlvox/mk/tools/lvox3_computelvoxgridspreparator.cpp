                                                                                   ...
                                                                                   int scannerRange)
{
    ...
    while (itGrp.hasNext())
    {
        ...
        if (scene && scanner)
        {
            ...
            //Extends scanner box based on scanner range if other than 0
            if(scannerRange != 0){
                //Accepts scanner range only in default option
                if(gridMode == lvox::BoundingBoxOfTheScene){
                    Eigen::Vector3d scannerRangeVector;
                    //Extends the scanner box for every +/- x,y coordinates
                    for(int i = 0;i<4;i++){
                        scannerRangeVector = tc.pattern->getCenterCoordinate();
                        if(i == 0){
                            scannerRangeVector.x() += scannerRange;
                        }else if(i==1){
                            scannerRangeVector.x() -= scannerRange;
                        }else if(i==2){
                            scannerRangeVector.y() += scannerRange;
                        }else{
                            scannerRangeVector.y() -= scannerRange;
                        }
                        scannerBox.extend(scannerRangeVector);
                    }
                }
            }
    ...
    if (!voxelBox.contains(scannerBox)) {
        ...
        if (gridMode==lvox::CenteredOnCoordinatesAndCustomDimensions) {
              ...
              while(v1(0)>v1s(0)) {v1(0) -= gridResolution;}
              while(v1(1)>v1s(1)) {v1(1) -= gridResolution;}
              while(v1(2)>v1s(2)) {v1(2) -= gridResolution;}
              while(v2(0)<v2s(0)) {v2(0) += gridResolution;}
              while(v2(1)<v2s(1)) {v2(1) += gridResolution;}
              while(v2(2)<v2s(2)) {v2(2) += gridResolution;}
              voxelBox.extend(v1);
              voxelBox.extend(v2);
        } else {
